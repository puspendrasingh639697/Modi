...
import { MdDiamond, MdLocalOffer, MdBlock, MdReport } from "react-icons/md";
import { FaCoins, FaEllipsisV } from "react-icons/fa";
...
export default function Chat() {
  ...
  }, [chatId, currentEmail, partnerEmail, messages.length, isChatDisabled]);

  // Improved send message function with better validation
  const handleSendMessage = () => {
    if (!inputText.trim()) return;
    
    if (!chatId || !currentEmail) {
      console.error("❌ Cannot send message - missing data:", {
        chatId,
        currentEmail,
        inputText: inputText.trim()
      });
      alert("Cannot send message. Chat information missing.");
      return;
    }
  ...
  // Message sending functions - IMPROVED
  const sendMessage = async (chatIdParam, text) => {
    if (!chatIdParam || !currentEmail || !text.trim()) {
  ...
  const sendMessageWithImage = async (chatIdParam, imageUrl) => {
    if (!chatIdParam || !currentEmail) {
      console.error("❌ Missing chatId or user for image message");
      return;
    }
  ...
  // Block user functionality - IMPROVED
  const handleBlockToggle = async () => {
    const userEmail = auth?.currentUser?.email;
    ...
    try {
      ...
      if (error && error.code !== "PGRST116") {
        console.error("❌ handleBlockToggle fetch error:", error);
        alert("Error", "Failed to fetch block status");
        return;
      }
      ...
      if (upsertError) {
        console.error("❌ handleBlockToggle update error:", upsertError);
        alert("Error", "Failed to update block status");
      } else {
        const newBlocked = !isBlocked;
        setIsBlocked(newBlocked);
        setIsChatDisabled(newBlocked);
  ...
  const handleAcceptCall = async () => {
    try {
      ...
      console.log('Error accepting call:', error);
      alert("Error", "Failed to accept call");
    }
  };
  ...
  const pickMedia = useCallback(() => {
    if (!chatId) {
      alert("Error", "Chat not available");
      return;
    }
  ...
  const sendMessageWithMedia = async (chatIdParam, mediaUrl, type) => {
    if (!chatIdParam || !currentEmail) {
      console.error("❌ Missing chatId or user for media message");
      return;
    }
  ...
  // Sticker sending - IMPROVED
  const handleSendSticker = async () => {
    if (!selectedSticker) return;
    
    if (!chatId || !currentEmail) {
      alert("Error", "Chat information missing");
      return;
    }
  ...
  const EmptyChat = () => (
    <div className="modern-empty-chat">
  ...
  // UI Components - IMPROVED WITH MENU BAR
  const Header = () => (
    <div className="modern-header">
      <div className="modern-header-content">
          ...
          <div className="modern-user-info">
            ...
            <div className="modern-user-details">
              <h2 className="modern-user-name">
                {params.name || params.email || "Unknown User"}
              </h2>
              <p className="modern-user-status">
                {isTyping ? (
                  <span className="modern-typing-indicator">
        ...
        <div className="modern-header-actions">
          {/* Voice Call Button */}
          <button
            className="modern-header-icon-button"
            onClick={() => {
              if (isChatDisabled) {
                alert("Blocked", "You can't call this user.");
              } else {
          ...
          <button
            className="modern-header-icon-button"
            onClick={() => setShowStickerPicker(true)}
            disabled={isChatDisabled || !chatId}
            title="Send Gift"
          >
            <IoIosGift size={20} />
          </button>
          ...
          <div className="modern-menu-container" ref={menuRef}>
            <button
              className="modern-header-icon-button"
            ...
            {showMenu && (
              <div className="modern-dropdown-menu">
                ...
                >
                  <IoIosPerson className="modern-menu-icon" />
                ...
                <button 
                  ...
                  onClick={handleBlockToggle}
                >
                  <MdBlock className="modern-menu-icon" />
                ...
                >
                  <MdReport className="modern-menu-icon" />
                ...
                >
                  <IoIosInformationCircle className="modern-menu-icon" />
        ...
        <div className={`modern-message-bubble ${isSelf ? "self" : "other"}`}>
          {message.image && (
            <div className="modern-media-container">
              <img 
                src={message.image} 
                className="modern-message-image" 
                alt="Shared content" 
                loading="lazy"
              />
              <div className="modern-media-overlay">🎁 Gift</div>
          ...
          {message.video && (
            <div className="modern-media-container">
              <video 
                src={message.video} 
                className="modern-message-video" 
                controls 
                preload="metadata"
              />
              <div className="modern-media-overlay">📹 Video</div>
              ...
              >
                <div className="modern-sticker-image-container">
                  <img 
                    src={it.imageUrl} 
                    alt={it.name} 
                    className="modern-sticker-image" 
                    loading="lazy"
                  />
                  {it.isNew && <span className="modern-new-badge">NEW</span>}
                  <div className="modern-sticker-cost">
    ...
    <div className="modern-chat-container">
      ...
      <div className="modern-input-container">
        ...
        <button
          className="modern-input-button modern-media-button"
          onClick={pickMedia}
          disabled={isChatDisabled || uploading || !chatId}
          title="Attach Media"
        >
          <IoIosImages size={24} />
        </button>
        ...
        <button
          ...
          disabled={!inputText.trim() || isChatDisabled || uploading || !chatId}
          title="Send Message"
        >
          <IoIosSend size={20} />
        </button>
